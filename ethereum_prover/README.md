...
The recursion verifier setup/layout artifacts are generated by `ethereum_prover`
from its current Airbender dependency. `zksync-os` remains the source of
`app.bin` / `app.text`, but should not be used as the source for verifier
artifacts.

```sh
cargo run --release -p ethereum_prover -- \
